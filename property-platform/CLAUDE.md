...
[`docs/РЪЧНИ-СТЪПКИ.md`](docs/РЪЧНИ-СТЪПКИ.md) е **за собственика**, не за
теб: всяка настройка, която не може да се направи от кода (ключове,
бутони във Vercel/Supabase/Resend/Google), написана с прости думи и
click-by-click. Собственикът не е програмист и работи със снимки на
екрана. **Когато въведеш нещо, което изисква негово ръчно действие,
допълни този файл** — иначе стъпката остава само в чата и се губи.
...
На началната страница се появява чак когато голямата картина в началото
излезе от екрана — иначе се дублират, а на телефон копчето ляга върху
бутона „Търси". Свързката е атрибутът **`data-hero-scene`**: носят го и
`HeroMark3D` (когато не е `compact`), и скрол сцената
(`story-scroll.tsx`). Помощникът го намира с `querySelector` и следи
позицията му при скрол. Затова **не махай атрибута**, ако пипаш началото.
...
### Началната страница: телефон и компютър вървят по различен път

⚠️ **Това е решено веднъж и с причина — не го „опростявай" обратно.**

Скрол сцената (по-долу) беше началото и на телефон. Собственикът прати
снимка с отворена клавиатура: два припокрити, полупрозрачни текста.
Причината: iOS сам превърта страницата, за да отвори клавиатурата, а
сцената реагира на скрол — картата с търсачката започва да чезне точно
докато човекът пише в нея. Отделно търсенето, обявите и „Публикувай"
оставаха чак на шести екран.

Затова началото сега е разделено по `lg` (1024px):

| Екран | Кой компонент | Какво вижда посетителят |
|---|---|---|
| под `lg` | `home/phone-hero.tsx` | един екран: снимка + търсачка, която **не мърда** |
| от `lg` | `home/story-scroll.tsx` | петте кадъра, както досега |

После, за всички екрани: `home/goal-tiles.tsx` („Какво търсиш?" — три
едри цели, третата тъмна), после „Последни обяви", и чак тогава
останалото.

⚠️ **На телефон историята я няма изобщо.** Имаше я за кратко като лента
с три кадъра (`story-strip.tsx`), но собственикът я махна — на малък
екран тя стоеше като чужда секция между обявите и сметката, а
разказът вече се носи от самите текстове по страницата. Историята
живее само в скрол сцената на широк екран. Не я връщай на телефон без
изрично искане.

Числата, заради които е сменено (телефон): екрани до търсачката
~1 (и избледняваща) → **0**; до първата обява 6 → **2**; до
„Публикувай" 5 → **1**.

⚠️ **Това разделяне беше връщано веднъж и после възстановено.**
Собственикът първо одобри тази подредба, после поиска движението пак на
телефон („върни стария вариант като скролвам да се променя"), а след
като го видя — пожела обратно лесната за ползване („което е
интегрирано за потребителя да е лесно за ползване"). Тоест текущото
състояние е обмислено и потвърдено два пъти. Ако темата се върне пак,
**не** предполагай — питай кое от двете, защото и двете вече са били
на живо.

⚠️ **Кадрите живеят в `home/story-frames.ts`, не в компонента.** Този
файл нарочно НЕ е `"use client"`: чете се и от сървърни компоненти
(`app/page.tsx`). Докато списъкът стоеше вътре в
клиентския `story-scroll.tsx`, Next.js подаваше на сървъра модулен
посредник вместо масива — `STORY_FRAMES[4]` излизаше `undefined` и
началната страница връщаше **500**. Ако пак разместваш общи данни между
сървър и клиент, това е капанът.

⚠️ **Скритата половина пак тегли снимки.** `display:none` не спира
браузъра — особено при `priority`, който добавя `<link rel=preload>`.
Затова и двата компонента носят `sizes` с `1px` за екраните, на които са
скрити (`sizes="(min-width: 1024px) 1px, 100vw"` и обратното): браузърът
взема най-малкия вариант от srcset-а, няколко килобайта вместо цял кадър.
Мързеливите снимки (без `priority`) и без това не се теглят, докато са в
скрит блок.

⚠️ **Плаващият помощник търси първата *видима* начална картина.**
Атрибутът `data-hero-scene` стои на две места и едното винаги е
`display:none`, тоест с височина 0 — тоест изглежда „вече подминато".
`ai-assistant.tsx` затова обхожда всички и взема първата с височина > 0;
ако още никоя няма размери, **не решава нищо** (иначе копчето изскача
върху бутона „Търси имот" и остава там до първия скрол). Има и
`ResizeObserver` върху `body`, защото височината се мени и без скрол —
идва шрифтът, зарежда се снимката, отваря се клавиатурата.

⚠️ **`scroll-pl-4` върви заедно с `px-4`** в лентата с кадрите. Без него
scroll-snap подравнява първия кадър до ръба на екрана, левият отстъп
изчезва и лентата изглежда изместена спрямо заглавието над нея.

Търсачката (`home/hero-search.tsx`) е една, с две облекла:
`glass` (полупрозрачна, върху снимката на широк екран) и `solid`
(плътно бяла карта на телефон, бутон „Търси имот" на цял ред, чиповете
се плъзгат настрани вместо да се редят на два реда).

### Скрол сцена: едно място през времето (`components/home/story-scroll.tsx`)

**От `lg` надолу тази секция е скрита** — виж разделянето по-горе.
Собственикът поиска началната страница да се движи „като видео“ при
скрол, по модел на pear.no, и отхвърли първия вариант (абстрактен 3D
град от кутийки) като недостатъчно живописен. Сегашният вариант е
**пет снимки от птичи поглед на едно и също място през времето** —
празна долина → първа къща → махала → градче → днешен град — които се
преливат една в друга при скрол, с бавно приближаване.

⚠️ **Нарочно НЕ е видео файл и НЕ е поредица от кадри.** Safari на
iPhone не превърта видео по скрол надеждно (заеква или замръзва), а
оттам идва голяма част от трафика. Пет снимки тежат около мегабайт;
видео със същото съдържание — десет. Преливането между снимки работи
навсякъде.

Снимките са **генерирани с Higgsfield** (`nano_banana_2`, 2 кредита на
кадър, 16:9, 2k). Кадри 2–5 са правени с кадър 1 като `image_references`,
за да не мърда долината и силуетът на планината. ⚠️ Това са
илюстративни картини, **не снимки на истинско място** — не бива да се
подписват с име на град. При смяна: генерира се наново със същия
първи кадър като референция, иначе серията се разпада.

⚠️ **Сесията не може да сваля от CDN-а на Higgsfield** (403 от
мрежовата политика, проверено два пъти). Затова адресите в
`STORY_FRAMES` (`components/home/story-frames.ts`) **временно сочат
направо натам**, а `next.config.ts` го допуска в
`images.remotePatterns`. Браузърът на посетителя не е зад тази
политика, а Vercel кешира оптимизираните копия на ръба.

Решение на собственика, взето след като рискът беше описан два пъти:
чужд CDN държи най-важната снимка на сайта. Изтече ли акаунтът или се
сменят адресите, началната страница остава без фон. **Да се замени** с
файлове в `public/hero/01.png` … `05.png` (собственикът ги качва — виж
docs/РЪЧНИ-СТЪПКИ.md, раздел A2), след което `remotePatterns` отпада.

Флагът `STORY_FRAMES_READY` в `src/app/page.tsx` гаси наведнъж всичко,
което зависи от кадрите: сцената на широк екран (връща се досегашният
hero), снимката в началото на телефон (остава само градиентът) и
лентата с трите кадъра. Аварийният изход с един ред, ако снимките се
счупят.

Снимките минават през `next/image` (`fill`, `priority` на първата),
затова могат да се качат като тежки PNG-та — Vercel ги сервира
преоразмерени и в webp/avif. Ако станат проблем за размера на repo-то,
конвертират се в webp с отделен commit.

⚠️ **Плавността идва от CSS преходи, не от по-често смятане.** Скролът
пристига на пресекулки (колелце, инерция) и без преход картината
прещраква между измерванията. Затова снимките, текстовите карти и
знакът носят `transition … ease-out` (320/300/620 ms) — браузърът сам
интерполира. Знакът е с най-дълъг преход нарочно: изостава една идея
след скрола и точно това го кара да изглежда, че плува над сцената. При
`prefers-reduced-motion` всички преходи отпадат (`calm`).
**Не слагай `will-change` на снимките** — пет цели екрана в отделни
слоеве ядат видеопаметта повече, отколкото печелят.

Механиката: секцията е висока 5 екрана, вътре има `sticky` слой с
петте снимки една върху друга. Скролът се смята **спрямо самата
секция** (`getBoundingClientRect().top`), не спрямо `scrollY` на
страницата — така работи и ако секцията един ден не е най-отгоре.
Прозрачността на всяка снимка е `1 - |курсор - индекс|`, което дава
верига от преливания без skok.
...
При под 3 активни обяви (`MIN_LISTINGS_FOR_FEED`) секцията „Последни
обяви" показва истинските обяви, които има, плюс една пунктирана покана
„Тук ще е твоята обява". Преди се скриваше изцяло — но откакто стои
високо на страницата, липсата ѝ оставяше впечатление за мъртъв сайт, а
една истинска обява до поканата е честна и покана, и доказателство.
Каквото и да става — **не** се рисуват измислени примерни имоти и не се
показват празни кутийки.
...
За рекламите във Facebook и Instagram. Dataset ID `1837636510591290`
стои в `NEXT_PUBLIC_FB_PIXEL_ID` (Config, не Secret — номерът се вижда
в кода на всяка страница и не е таен). Без зададена променлива
пикселът **не се зарежда изобщо** — нормалното състояние за dev.

| Част | Файл |
|---|---|
| Зареждане на скрипта + PageView при смяна на маршрут | `components/meta-pixel.tsx` |
| `pixelEvent()` / `pixelPageView()` | `lib/fpixel.ts` |
| Еднократно събитие от сървърна страница | `components/pixel-event.tsx` |
| `Contact` при натискане на телефона | `components/phone-link.tsx` |

Кои събития се пращат и откъде:

| Събитие | Къде |
|---|---|
| `PageView` | базовият скрипт (първо зареждане) + `PixelTracker` при всяка смяна на маршрут |
| `ViewContent` | отваряне на обява (`app/listings/[id]/page.tsx`) |
| `Search` | търсачката в началото (`home/hero-search.tsx`) и всяка промяна на филтър (`listing-filters.tsx`) |
| `CompleteRegistration` | `/dashboard?registered=1` след успешна регистрация |
| **`Lead`** | **контакт с обявителя** — натиснат телефон (`phone-link.tsx`) или първо съобщение в разговор (`message-thread-form.tsx`) |
| `SubmitApplication` | `/listings/[id]?published=1` след публикуване на обява |

⚠️ **`Lead` е контактът, не публикуването.** Първоначално `Lead` беше
на публикуването и това е грешка, която струва пари: в Events Manager
двете публики се смесват — тези, които търсят имот, и тези, които
предлагат. Meta оптимизира рекламата към едната и я показва на
другата. Затова публикуването е `SubmitApplication` (също стандартно
събитие, също влиза в Aggregated Event Measurement).

⚠️ **`Lead` от съобщение се праща само за първото съобщение** в
разговора от този потребител (`isFirstContact`, което идва от
страницата — само тя знае историята). Иначе една дълга кореспонденция
излиза като двайсет отделни контакта.

⚠️ **`Subscribe` няма къде да се сложи.** В проекта няма плащания —
плановете и `/pricing` бяха премахнати (виж „Бизнес модел" по-горе) и
реално плащане никога не е свързвано. Ако някой ден се върне
абонамент, мястото е успешният webhook от платеца, не клиентът:
събитие от браузъра се подправя от всеки.

⚠️ **Пикселът слага бисквитка и това променя правния статус на сайта.**
Vercel Analytics беше избран нарочно, защото не слага бисквитки и сайтът
нямаше нужда от cookie банер (виж „Web Analytics" по-горе). Meta Pixel
слага `_fbp` и праща данни на трета страна извън ЕС — по GDPR и чл. 4б
от ЗЕС това изисква **предварително съгласие** от посетителя. Към
момента съгласие **не се иска** — пикселът тръгва веднага. Това е
известно и е решение на собственика, взето след като рискът беше
описан. Когато се добави лента за съгласие, единственото място за
промяна е `enabled` в `components/meta-pixel.tsx`.

⚠️ **Никога не пращай лични данни през `pixelEvent`** — имейл, телефон,
име, ЕГН, точен адрес. Затова `PhoneLink` праща id на обявата и града,
но НЕ самия номер. Meta би приела и номера, но това е предаване на
лични данни на трета страна извън всичко, което сме казали на
потребителите.

⚠️ **Известна дупка: регистрацията през Google не се брои.**
`CompleteRegistration` се праща само по пътя с имейл и парола
(`signUp` пренасочва към `/dashboard?registered=1`). OAuth минава през
`app/auth/callback/route.ts`, откъдето нов потребител не се различава
от връщащ се без допълнителна заявка. Ако това стане важно за
оптимизацията на рекламите, най-чистото е `handle_new_user()` да
отбележи момента и callback-ът да го прочете.

⚠️ **Двойният `PageView`** е класическият капан на App Router: базовият
скрипт праща един при зареждане, а `useEffect` в `PixelTracker` праща
втори при монтиране. Затова първото извикване на ефекта се пропуска
(`firstRun`). Проверено: при зареждане опашката е `init` + един
`PageView`, при навигация — точно един нов.

⚠️ `useSearchParams` кара всичко над себе си да се рендира в браузъра,
ако не е зад `Suspense`. Затова следенето живее в отделен вътрешен
компонент.

Проверка след deploy: Chrome разширението **Meta Pixel Helper** трябва
да светне зелено на imotpoint.com; Events Manager → dataset → **Test
...
⚠️ **Мерките срещу агенции НЕ се описват в публичните текстове.** По
решение на собственика (конкуренцията чете сайта) `/about` и системният
текст на AI помощника казват само, че има проверки — автоматични и ръчни
— и че потвърдените посредници се блокират, без да изброяват кои са
мерките. Подробен списък е и готово упътване как да се заобиколят.
Лимитът от 3 обяви на профил стои в секцията "Колко струва" като
обикновено правило за ползване, не като мярка срещу агенции. Ако някой
ден се добавя текст тук — твърдение, не механика.
...
## Viber робот — какво чете и какво НЕ може да прочете

Собственикът поиска нещо, което следи Viber кореспонденцията му и казва кой
чака отговор. Проверено на живата му машина, **до текста на съобщенията няма
път**: `viber.db` е SQLCipher-криптирана (ключовете за E2E са в съседния
`data.db`), Viber Desktop е на Qt и не излага съдържание за достъпност (24
елемента, 5 текста = само рамката на прозореца), а iOS забранява четене на
чужди известия. Остава единствено това, което Viber **рисува**.

Затова веригата е: робот на Mac-а снима прозореца → `/api/viber/ingest`
разчита снимката с Haiku → `viber_chats` / `viber_observations` →
`/dashboard/viber`. Роботът е в `viber-assistant/agent/` (чист bash, само
вградени в macOS команди, нула инсталации).

| Част | Къде |
|---|---|
| роботът | `viber-assistant/agent/viber-agent.sh` |
| слагане на робота | `viber-assistant/agent/install.sh` |
| приемане на снимка | `src/app/api/viber/ingest/route.ts` |
| таблото | `src/app/dashboard/viber/page.tsx` |
| схема + RLS | `supabase/migrations/0027_viber_watch.sql` |
| таван на разхода | `0028_viber_spend_cap.sql` |
| писмо при 5 € | `0029_viber_spend_alert.sql` |
| три състояния на подателя | `0030_viber_reading_fix.sql` |
| питане с думи | `src/app/api/viber/ask/route.ts` + `components/viber-ask.tsx` |
| таван на въпросите | `0031_viber_ask.sql` |

⚠️ **Спирачката трябва да е ПРЕДИ разхода.** Първата версия проверяваше тавана
след извикването на модела — парите вече са похарчени, а заявката се отхвърля
след това. Оттам идва `viber_claim_slot`: иска разрешение, после се харчи.
Редът е същината, не числото.

⚠️ **`viber_ingest` е с ДВА аргумента.** 0027 я създаде с три (третият беше
интервалът), 0028 премести интервала в `claim_slot` и дропна стария подпис.
0030 за малко да го върне — тогава в базата стоят две едноименни функции и
PostgREST отказва: *"function viber_ingest(text, jsonb) is not unique"*.
Хванато с пускане срещу истински Postgres 16, не на око.

⚠️ **Българският Viber НЕ пише "Вие:" пред своите съобщения.** Указанията към
модела първоначално разчитаха точно на това и проверката излезе безсмислена —
в снимка от истински акаунт такава представка няма нито веднъж. Своето
съобщение се познава по отметките ✓✓ до реда, а реакциите се изписват с
"Ти реагира с …". Оттам и `sips -Z 1568` вместо 1100: отметките са няколко
точки и под тази големина не се различават.

⚠️ **"Не знам" не бива да се излива в "клиентът чака".** Докато полето беше
boolean, несигурността ставаше `false`, тоест "чака отговор", и на живо 16 от
17 разговора излязоха чакащи — число, което описваше стойността по
подразбиране, а не пощенската кутия. Сега подателят е `me` / `them` /
`unclear`, а `unclear` **не пипа** `waiting_since`: нито го пуска, нито го
спира. Таблото показва отделен раздел "Не е ясно". Ако някой ден това се
"опрости" обратно до две стойности, дефектът се връща цял.

⚠️ **Списъкът съдържа канали и групи, не само клиенти.** Автокъщи и официални
акаунти трупат стотици непрочетени; там "последното съобщение е чуждо" е
нормално, не сигнал. Затова `viber_chats.kind` дели `person` от `group` и
таблото ги брои отделно.

⚠️ **Снимката е на целия прозорец, включително отворения разговор.** Тоест към
модела отива повече, отколкото се записва в базата (в базата влиза само редът
от списъка). Изрязване само на левия списък е следващата стъпка и изисква
мъничка помощна програма — `sips` не може да реже с отместване.

⚠️ **Питането е на Opus, разчитането на Haiku — нарочно.** Разчитането на
списък е механично и върви десетки пъти на ден, тоест цената се умножава;
въпросът е рядък и иска разбиране. Двете задачи не делят един избор и имат
отделни броячи (`viber_agents` за робота, `viber_ask_quota` за въпросите) —
смесването им би скрило кой от двата разхода се е разприказвал. Всеки има свой
праг от 5 евро.

⚠️ **Дневникът се подава на модела като ПРОМЕНИ, не като редове.** Роботът
записва ред при всяко наблюдение, дори когато нищо не се е сменило — за един
разговор това са десетки еднакви реда на ден. Подадени както са, изяждат
контекста и карат модела да мисли, че е имало десетки съобщения. `changesOnly()`
в `api/viber/ask/route.ts` оставя само момента, в който откъсът наистина се е
променил. Това е и единственото, което прилича на история: пунктирана следа на
всеки 20 минути, не препис. Системният текст изрично го казва на модела, защото
уверено допълнен разговор е по-вреден от "виждам само това".

⚠️ **Разрешението за запис на екрана се дава на приложението, по подпис.**
launchd пуска `/bin/bash`, на когото разрешение не се дава — затова роботът е
опакован като `ViberRobot.app`. И понеже macOS помни разрешението по подписа,
в приложението стои само непроменлив стартер, а роботът живее отвън в
`~/.viber-agent/viber-agent.sh`. Пресъздаване на бъндъла = ново разрешение от
собственика; затова `install.sh` го пипа само когато версията на стартера
наистина се смени.
