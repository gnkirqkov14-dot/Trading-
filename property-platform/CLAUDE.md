...
в браузъра (не се пазят в базата), един неутрален топъл тон
(`NEIGHBORHOOD_FILL`) за всички клетки (не пъстра палитра — виж по-долу),
с постоянен label (името) в центъра на клетката — CSS override в
`globals.css` (`.leaflet-tooltip.map-label`) маха стандартната бяла
Leaflet tooltip кутийка, за да изглежда като директен надпис върху
областта.

**Органична външна форма, не правоъгълник**: първата версия клипваше
Voronoi диаграмата към ос-подравнен правоъгълник (padding около
bounding box-а на точките) — изглеждаше като кварталите да седят в
изкуствена кутия, собственикът основателно го намери за грозно.
Вместо това клипваме към **разширената изпъкнала обвивка** (convex hull,
`delaunay.hull`, избутана навън от центроида си с `HULL_EXPAND=1.35`) чрез
ръчна Sutherland-Hodgman полигонна клипваща функция
(`clipToConvexPolygon`) — резултатът следва естественото разпределение на
точките, изглежда като органичен силует на града. **Капан, в който вече
паднахме веднъж**: `delaunay.hull`-ът НЕ гарантира конкретна посока на
завъртане (CW или CCW) — зависи от разположението на точките. Ако
clip-полигонът е по посока на часовниковата стрелка, `isLeftOf()`
тестът излиза наопаки и ВСяка точка се clip-ва като "отвън", давайки
напълно празна карта без грешка в конзолата (тих failure, не exception!).
Затова кодът винаги проверява signed area на hull-а и го обръща ако е
отрицателен (CW), преди clipping-а.

**Защо не истински полигонни граници**: проверено директно (WebSearch) —
няма свободно достъпен, прецизен GeoJSON източник за границите на
неформалните квартали (за разлика от официалните области/`provinces`, за
които Natural Earth работи чудесно). Единственото близко нещо е
`yurukov/Bulgaria-geocoding` repo-то с полицейски RPU-зони само за
София (не покрива Пловдив/Варна/Бургас, и вече е отхвърлено по-горе
заради липсваш license) — не е добро 1:1 съответствие с имената на
seed-натите квартали. По-добре честно приблизителна диаграма, отколкото
измислени координати, представени като "истински".

За градове с само 1 квартал (Voronoi няма смисъл с 1 точка) остава
старият точков маркер.

**Закръглени "на ръка" ръбове (Chaikin corner-cutting)**: дори с
органичната hull-форма отгоре, самите клетки все още имаха прави ъглови
ръбове — собственикът основателно го оприлича на "резени пица" след като
намери реална снимка на кварталите на Пловдив (закръглени, органични
форми). `chaikinSmooth()` в `city-map.tsx` закръгля всеки полигон с 3
итерации corner-cutting (всеки ръб се разделя на 25%/75% точки, замествайки
острите ъгли) — резултатът изглежда нарисуван на ръка, не геометрична
диаграма. Приложено СЛЕД hull clipping-а, преди converting към latLngs.

### Ръчно пресъздадени форми за Пловдив (`public/data/plovdiv-districts.geojson`)

Собственикът намери реална снимка на официалната карта на кварталите на
Пловдив и поиска същите имена и приблизително същите форми/разположение
— не изчислена Voronoi диаграма. Понеже (виж по-горе) няма свободно
достъпен прецизен полигонен източник, формите в този файл са **ръчно
пресъздадени** (по визуална оценка от снимката, не survey-прецизни) от 13
реални квартала: Северна индустриална зона, Кършияка, Изгрев, Източна
индустриална зона, Център, Широк център, Тракия, Прослав, Смирненски,
Въстанически, Индустриална зона Юг, Коматево, Гагарин.
`supabase/migrations/0016_plovdiv_real_districts.sql` подравнява lat/lng
на съществуващите 5 квартала към новия layout и добавя останалите 8.

`city-map.tsx` проверява `HAND_AUTHORED_DISTRICTS[city.name]` **преди**
да извика Voronoi изчислението — ако има GeoJSON файл за града (в момента
само Пловдив), зарежда го и рисува реалните полигони по име на квартал
(join между `feature.properties.name` и `neighborhood.name` от базата).
Само за градове БЕЗ запис в `HAND_AUTHORED_DISTRICTS` пада обратно на
Voronoi изчислението. **Ако добавяш нов град по този начин**: снимка на
официалната карта → ръчно оцени нормализирани (0-100) координати на
всеки квартал → нов `{град}-districts.geojson` в `public/data/` →
migration с новите lat/lng centroids → добави реда в
`HAND_AUTHORED_DISTRICTS`.
...
По искане на собственика, следвайки imot.bg визуално 1:1: под картата на
всяко ниво (области → градове в `bulgaria-map.tsx`, град → квартали в
`app/map/[cityId]/page.tsx`) има и обикновен списък с имена като
бутони/линкове, синхронизиран с картата — не само карта. Това реши и
