...
⚠️ **Сесията не може да сваля от CDN-а на Higgsfield** (изходът е
блокиран от мрежовата политика — 403 от прокси-то). Файловете минават
през собственика: сваля ги от Higgsfield и ги качва в
`public/hero/01.png` … `05.png`. Затова има флаг:

```ts
const STORY_FRAMES_READY = false;   // src/app/page.tsx
```

Докато е `false`, началната страница показва досегашния hero. Обръща се
на `true` в мига, в който петте файла са в repo-то — по-добре старият
вид, отколкото пет счупени картинки.
