...
### Началната страница: подредбата под сцената

Скрол сцената е началото **и на телефон, и на компютър**. Под нея, за
всички екрани, идват:

1. **„Какво търсиш?"** (`home/goal-tiles.tsx`) — три едри цели:
   продажби, наеми и „Публикувай обява" (тъмната). Публикуването е
   единственото, което пълни сайта, и единственото, което не се случва
   само̀ — затова е отделено визуално.
2. **„Последни обяви"** — веднага след това, не в дъното. Обявите са
   доказателството, че сайтът е жив.
3. Чак тогава лентата с показателите, „Как работи", сметката и призивът.

⚠️ **История на едно връщане — за да не се прави пак по погрешка.**
Веднъж сцената беше скрита на телефон и заменена с един неподвижен
екран (снимка + търсачка), защото собственикът прати снимка с отворена
клавиатура: два припокрити, полупрозрачни текста. Причината е реална и
**не е поправена** — iOS сам превърта страницата, за да отвори
клавиатурата, сцената реагира на скрол и картата с търсачката започва
да чезне точно докато човекът пише в нея.

Собственикът обаче поиска движението обратно („върни стария вариант
като скролвам да се променя") и това е неговото решение. Ако темата се
върне, **не** махай сцената пак — по-малката поправка е търсачката да
не участва в избледняването (да стои в неподвижен слой над сцената или
веднага под нея), а картината да продължи да се сменя.

Кадрите живеят в `home/story-frames.ts`. ⚠️ Този файл нарочно НЕ е
`"use client"`: чете се и от сървърни компоненти. Докато списъкът стоеше
вътре в клиентския `story-scroll.tsx`, Next.js подаваше на сървъра
модулен посредник вместо масива — `STORY_FRAMES[4]` излизаше
`undefined` и началната страница връщаше **500**.

⚠️ **Плаващият помощник търси първата *видима* начална картина.**
`ai-assistant.tsx` обхожда всички `data-hero-scene` и взема първата с
височина > 0; ако още никоя няма размери, **не решава нищо** (иначе
копчето изскача върху бутона „Търси" и остава там до първия скрол).
Има и `ResizeObserver` върху `body`, защото височината се мени и без
скрол — идва шрифтът, зарежда се снимката, отваря се клавиатурата.
...
Флагът `STORY_FRAMES_READY` в `src/app/page.tsx` пали/гаси цялата
секция. При `false` се връща досегашният hero — аварийният изход с един
ред, ако нещо се счупи.
...
верига от преливания без скок.

⚠️ **Плавността идва от CSS преходи, не от по-често смятане.** Скролът
пристига на пресекулки (пръст, колелце, инерция) и без преход картината
...
интерполира между две стойности. Знакът е с най-дълъг преход нарочно:
изостава една идея след скрола и точно това го кара да изглежда, че
плува над сцената, вместо да е залепен за пръста. При
`prefers-reduced-motion` всички преходи отпадат (`calm`).

⚠️ **Не слагай `will-change` на снимките.** Пет цели екрана, вдигнати в
отделни слоеве, ядат видеопаметта на телефона повече, отколкото
печелят; браузърът и без това вдига слой, докато преходът тече.

Орбитата на знака е по-широка на тесен екран (`rx` 29 / `ry` 14 срещу
15/25 на широк), иначе на 390px той само потрепва на едно място и не се
чете като движение.
