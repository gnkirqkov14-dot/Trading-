...
сочейки към `SUPPORT_EMAIL` (виж `listing-labels.ts` — засега **placeholder**,
трябва да се смени с реален наблюдаван адрес, след като собственикът
настрои приемащ имейл на `imotspot.com`). `/dashboard/listings/new`
проверява лимита предварително и показва банер вместо формата, ако е
достигнат — потребителят не пълни цялата форма само за да гръмне накрая.
