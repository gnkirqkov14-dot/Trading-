...
- **`hidden lg:block` НЕ спира React effect-а.** Първата версия теглеше
  ...
  Затова breakpoint-ът се проверява и в кода
  (`matchMedia("(min-width: 1024px)")`), не само в CSS. Проверено срещу
  production build: на 390px не се тегли нищо и няма canvas.
