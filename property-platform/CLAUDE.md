...
- **Наблюдение на Viber кореспонденцията — правено и МАХНАТО по искане на
  собственика** (миграция `0033_viber_removed.sql`). Веригата работеше от
  край до край: робот на Mac-а снимаше прозореца, сървърът разчиташе
  снимката с Haiku, таблото показваше кой чака отговор. Спря се, защото
  разчитането не е достатъчно точно, за да се вярва на резултата. Трите
  причини, ако темата се върне: (1) българският Viber **не пише "Вие:"**
  пред своите съобщения — единственият признак кой е писал последен са
  отметките ✓✓; (2) те са няколко точки и се губят при смаляване на
  снимката; (3) едно и също име се разчита по няколко начина и един
  разговор става на три. Кодът е в git историята (миграции 0027–0032,
  `viber-assistant/`, PR #79–#82). Личният акаунт на Viber няма API —
  проверено: базата е криптирана, Qt не излага съдържание за достъпност,
  а iOS забранява четене на чужди известия. Тоест път през снимка е
  единственият възможен, не избор.
