...
# Скрива потребителското име И телефонния номер (Viber кръщава папката на него),
# за да може изходът наистина да се споделя.
short_path() {
  local t='~'
  printf '%s' "${1/#$HOME/$t}" | sed -E 's#/[0-9]{8,}(/|$)#/<номер>\1#g'
}
