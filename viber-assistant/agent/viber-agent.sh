...
one_round() {
  ...
  # Намаляваме до 1568 — точно колкото моделът ползва. Retina дава над 2300
  # точки ширина и горницата се изхвърля от самия модел, тоест се плаща за
  # нищо. sips е вграден в macOS, не се инсталира нищо.
  #
  # Беше 1100 и това се оказа под границата на четимото. На живо излезе, че
  # българският Viber НЕ пише "Вие:" пред своите съобщения — единственият
  # признак кой е писал последен са отметките ✓✓ до реда, а те са няколко
  # точки. При 1100 моделът не ги различаваше, връщаше "клиентът писа
  # последен" за почти всичко и таблото показваше измислени чакащи.
  # Разликата в цената е около една стотинка на сто снимки; разликата в
  # смисъла е между работещо и подвеждащо табло.
  sips -Z 1568 "$shot" --out "$shot" >/dev/null 2>&1 || true
